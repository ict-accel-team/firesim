...
case class MMIOLatencyPipeConfig(params: BaseParams) extends BaseConfig{
  def elaborate()(implicit p:Parameters): MMIOLatencyPipe = Module(new MMIOLatencyPipe(this))
}

class MMIOLatencyPipeMMRegIO(cfg: BaseConfig)(implicit p: Parameters) extends SplitTransactionMMRegIO(cfg){
  val readLatency = Input(UInt(32.W))
  val writeLatency = Input(UInt(32.W))

  val registers = maxReqRegisters ++ Seq(
    (writeLatency -> RuntimeSetting(2, "Write latency", min = 1)),
    (readLatency  -> RuntimeSetting(2,"Read latency", min = 1))
  )
...
class MMIOLatencyPipeIO(val cfg: MMIOLatencyPipeConfig)(implicit p: Parameters) extends SplitTransactionModelIO()(p) {
  val mmReg = new MMIOLatencyPipeMMRegIO(cfg)
}


class MMIOLatencyPipe(cfg: MMIOLatencyPipeConfig)(implicit p: Parameters) extends SplitTransactionModel(cfg)(p) {
  lazy val io = IO(new MMIOLatencyPipeIO(cfg))

  val longName = "MMIO Latency Bandwidth Pipe"
  def printTimingModelGenerationConfig {}
  /**************************** CHISEL BEGINS *********************************/
  // Configuration values
  val readLatency = io.mmReg.readLatency
