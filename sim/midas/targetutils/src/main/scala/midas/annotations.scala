...
object PerfCounter {
  ...
  def apply(
    ...
    emitAnnotation(target, clock, reset, label, message, opType)
  ...
  def apply(target: chisel3.UInt, label: String, message: String): Unit =
    emitAnnotation(target, Module.clock, Module.reset, label, message, PerfCounterOps.Accumulate)
  ...
  /**
    ...
    *
    */
  def identity(target: chisel3.UInt, label: String, message: String): Unit = {
    ...
    emitAnnotation(target, Module.clock, Module.reset, label, message, opType = PerfCounterOps.Identity)
  }
}
