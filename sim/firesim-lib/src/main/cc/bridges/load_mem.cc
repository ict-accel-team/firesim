...
custom_loadmem_t::custom_loadmem_t(simif_t* sim, const std::vector<std::string>& args, bool has_mem, int64_t mem_host_offset):
        sim(sim), has_mem(has_mem), mem_host_offset(mem_host_offset) {
    std::string num_equals = std::to_string(0) + std::string("=");
    ...
    printf("Using custom_loadmem_t to load workload ...");
    printf("\n");

    fesvr = new firesim_tsi_t(argc_count+1, argv_arr, has_mem);
}

custom_loadmem_t::~custom_loadmem_t() {
    delete fesvr;
}

void custom_loadmem_t::check_loadmem_done() {
    while(fesvr->data_available()) {
        if(fesvr->recv_word() == 0x2000000){
            loadmem_done = true;
            break;
        }
    }
...
void custom_loadmem_t::handle_loadmem_read(firesim_loadmem_t loadmem) {
...
void custom_loadmem_t::handle_loadmem_write(firesim_loadmem_t loadmem) {
...
void custom_loadmem_t::serial_bypass_via_loadmem() {
...
void custom_loadmem_t::loadmem_by_serial(){
    while(!loadmem_done){
    	this->tick();
    }
}

void custom_loadmem_t::tick() {
    // Punt to FESVR
    ...
    if (!loadmem_done) {
        // check if tsi sends target reset
        this->check_loadmem_done();
    }
}
